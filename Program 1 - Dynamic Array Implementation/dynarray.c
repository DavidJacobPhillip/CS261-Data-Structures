...
struct dynarray* dynarray_create() {
  struct dynarray *new_struct = malloc(sizeof(struct dynarray));
  new_struct->data = malloc(2*sizeof(void*)); 
  new_struct->capacity=2;
  new_struct->size=0;
  return new_struct;
}
...
void dynarray_insert(struct dynarray* da, void* val) {
  if (da->capacity == da->size){
    void** new_array = malloc(2*da->capacity*sizeof(void*));
    for (int i = 0; i < da->size; i++){
      new_array[i] = da->data[i];
    }
    free(da->data);
