...
// ADDITIONAL FUNCTIONS ADDED BY ME
 /*
  * Removes the first element of the list and frees it
  */
  void* pop_list(struct list* list){
    if (list->head != NULL){
      struct link* after = list->head->next;
      void* value = list->head->val;
      free(list->head);
      list->head = after;
      return value;
    }
 ...
 /*
  * Determines if the list is empty or not, returning a 1 if its empty and 0 otherwise;
  */
 ...
 /*
  * Returns the top value of the list head element
  */
  void* head_value(struct list* list){
    if(list->head != NULL){
      // int *convert = list->head->val;
      // printf("value: %d", *convert);
