...
// ADDITIONAL FUNCTIONS ADDED BY ME
  ...
  */
  int array_empty(struct dynarray* da){
    if (da->size == 0){
      return 1;
    }
    return 0;
  ...
  */
  void* start_element(struct dynarray* da){
    return da->data[da->start];
  }

 /*
  * Removes the front_most element
  */
  void* remove_start(struct dynarray* da){
    void* value = da->data[da->start];
    da->start++;
    da->size--;
